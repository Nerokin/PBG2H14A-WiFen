...
public class Spielfeld implements MarkerService {
	...
	private Spielfeld(double sizeFieldX, double sizeFieldY, double sizeSceneX, double sizeSceneY, int tilesPerRow,
			int tilePerCol, int type, MarkerView markerWindow) {
	...
	/**
	 * initialize a field with a rectangle grid sizeFieldX => Stack width
	 * sizeFieldY => Stack height sizeSceneX => Scene width sizeSceneY => Scene
	 * height tilesPerRow => rectangle per row tilesPerCol => rectangle per
	 * column
	 */
	public Spielfeld(double sizeFieldX, double sizeFieldY, double sizeSceneX, double sizeSceneY, int tilesPerRow,
			int tilesPerCol, MarkerView markerWindow) {
		this(sizeFieldX, sizeFieldY, sizeSceneX, sizeSceneY, tilesPerRow, tilesPerCol, 1, markerWindow);
	}

	/**
	 * initialize a field with a hexagon grid sizeFieldX => Stack width
	 * sizeFieldY => Stack height sizeSceneX => Scene width sizeSceneY => Scene
	 * height tilesPerCol => hexagons per column
	 */
	public Spielfeld(double sizeFieldX, double sizeFieldY, double sizeSceneX, double sizeSceneY, int tilesPerCol,
			MarkerView markerWindow) {
		this(sizeFieldX, sizeFieldY, sizeSceneX, sizeSceneY, 0, tilesPerCol, 2, markerWindow);
	}

	/**
	 * initialize a field without a grid sizeFieldX => Stack width sizeFieldY =>
	 * Stack height sizeSceneX => Scene width sizeSceneY => Scene height
	 */
	public Spielfeld(double sizeFieldX, double sizeFieldY, double sizeSceneX, double sizeSceneY,
			MarkerView markerWindow) {
	...
	public void draw() {
		...
		if (this.typ == 1) {
			...
			line.autosize();
			line.getPoints().addAll(new Double[] { 0.00, 0.00, 0.00, scrollPane.getHeight() });
			for (int i = 0; i < tilesPerCol; i++) {
				for (int j = 0; j < tilesPerRow; j++) {
					line.getPoints().addAll(new Double[] { j * a, i * b, (j + 1) * a, i * b, (j + 1) * a, (i + 1) * b,
							j * a, (i + 1) * b, j * a, i * b });
				}
				line.getPoints().addAll(new Double[] { (tilesPerRow - 1) * a, (i + 1) * b });
			}
			this.add(line);
		} else if (this.typ == 2) {
			boolean toggle = true;
			...
			for (int i = 0; i < tilesPerCol; i++) {
				if (toggle) {
					for (int j = 0; j < tilesPerRow; j++) {
						line.getPoints()
								.addAll(new Double[] { (((2 * (c + a)) * j)),
										(i * (2 * b))
												+ b,
										(a + ((2 * (c + a)) * j)), i * (2 * b), (a + c + ((2 * (c + a)) * j)),
										i * (2 * b),
										((2 * a + 2 * c) * j)
												+ (2 * a + c),
										(i * (2 * b))
										...
										(a + ((2 * (c + a)) * j)), (i * (2 * b)) + (2 * b), (((2 * (c + a)) * j)),
										(i * (2 * b)) + b, (a + ((2 * (c + a)) * j)), i * (2 * b),
										(a + c + ((2 * (c + a)) * j)), i * (2 * b), ((2 * a + 2 * c) * j) + (2 * a + c),
										(i * (2 * b)) + b });
					}
					if (i < tilesPerCol - 1) {
						line.getPoints()
								.addAll(new Double[] { (a + c + ((2 * (c + a)) * (tilesPerRow - 1))),
										(i * (2 * b)) + (2 * b), ((2 * a + 2 * c) * (tilesPerRow - 1)) + (2 * a + c),
										((i + 1) * (2 * b)) + b, });
						toggle = false;
					}
				} else {
					for (int j = tilesPerRow - 1; j >= 0; j--) {
						line.getPoints()
								.addAll(new Double[] { ((2 * a + 2 * c) * j) + (2 * a + c), (i * (2 * b)) + b,
										(a + c + ((2 * (c + a)) * j)),
										(i * (2 * b))
												+ (2 * b),
										(a + ((2 * (c + a)) * j)),
										...
										(i * (2 * b))
												+ b,
										(a + ((2 * (c + a)) * j)), i * (2 * b), (a + c + ((2 * (c + a)) * j)),
										i * (2 * b), ((2 * a + 2 * c) * j) + (2 * a + c), (i * (2 * b)) + b,
										(a + c + ((2 * (c + a)) * j)), i * (2 * b), (a + ((2 * (c + a)) * j)),
										i * (2 * b), (((2 * (c + a)) * j)), (i * (2 * b)) + b });
					}
					if (i < tilesPerCol - 1) {
						line.getPoints().addAll(new Double[] { (a + ((2 * (c + a)) * 0)), ((i) * (2 * b)) + (2 * b),
								(a + ((2 * (c + a)) * 0)), (i + 1) * (2 * b) });
						toggle = true;
					}
				}
			...
			public void handle(MouseEvent event) {
				if (!hasDragged && event.getButton() == MouseButton.PRIMARY) {
					if (event.getX() > pane.getWidth() || event.getY() > pane.getHeight()) {
						...
						// beinhaltet Neurendern der ScrollPane und aller
						// Children, damit Marker korrekt nach dem Erstellen
						// angezeigt werden
	...
	/**
	 * resize the grid. sizeFieldX for the new X Value. sizeFieldY for the new Y
	 * Value.
	 */
	...
	/**
	 * resize the window. sizeSceneX for the new X Value. sizeSceneY for the new
	 * Y Value.
	 */
	...
	/**
	 * Return the type of marker currently selected in the marker-selection
	 * window.
	 */
	@Override
	public MarkerType getSelectedType() {
		...
		return new MarkerType("mockup");
	}

	/**
	 * Places one marker of the currently selected type at the mouse's current
	 * position.
	 */
	@Override
